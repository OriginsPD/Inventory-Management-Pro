  ...
  try {
    if (url) {
      ...
      try {
        ...
        console.warn(
          "⚠️ Database migration history is missing, but schema objects already exist. Validating schema compatibility...",
        );
      }
    }
    ...
    try {
      const bootstrapEmail = env.IMS_BOOTSTRAP_ADMIN_EMAIL;
      const bootstrapPassword = env.IMS_BOOTSTRAP_ADMIN_PASSWORD;
      if (!bootstrapEmail || !bootstrapPassword) {
        console.warn(
          "Bootstrap admin seed skipped. Set IMS_BOOTSTRAP_ADMIN_EMAIL and IMS_BOOTSTRAP_ADMIN_PASSWORD for first-run setup.",
        );
        return;
      }
    ...
    if (env.NODE_ENV === "production" && env.DATABASE_URL) {
      console.error(
        "🚨 [IMS API] Database is configured but connection failed. Crashing server to prevent silent data loss.",
      );
      process.exit(1);
    }
