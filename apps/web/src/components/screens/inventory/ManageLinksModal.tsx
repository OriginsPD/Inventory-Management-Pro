      ...
      <DialogContent className="glass-panel-elevated rounded-2xl max-w-lg w-full max-h-[90vh] border-0" showCloseButton={true}>
        ...
        <div className="space-y-6 text-left">
          ...
          <div className="space-y-2 border-t border-primary/10 pt-4">
            ...
            {parentDev ? (
              <div className="border border-primary/10 rounded-xl bg-primary/5 p-2.5 px-3 flex justify-between items-center text-xs">
              ...
              <div className="space-y-2">
                <input type="text" placeholder="Search compatible parent devices..." value={linkParentSearch} onChange={(e) => setLinkParentSearch(e.target.value)} className="flex h-9 w-full rounded-lg border border-primary/10 bg-background/50 px-3 text-xs text-foreground" />
                <ScrollArea className="border border-primary/10 rounded-xl max-h-32 bg-background">
          ...
          {allowedChildTypes.length > 0 && (
            <div className="space-y-2 border-t border-primary/10 pt-4">
              ...
              {(childRels.length > 0 || stagedChildren.length > 0) && (
                <div className="border border-primary/10 rounded-xl bg-primary/5 overflow-hidden divide-y divide-primary/10">
                  {childRels.map(rel => {
              ...
              <input type="text" placeholder="Search compatible child components..." value={linkSearch} onChange={(e) => setLinkSearch(e.target.value)} className="flex h-9 w-full rounded-lg border border-primary/10 bg-background/50 px-3 text-xs text-foreground" />
              <ScrollArea className="border border-primary/10 rounded-xl max-h-32 bg-background">
          ...
          <div className="flex gap-2 justify-end pt-4 border-t border-primary/10">
            <button type="button" onClick={onClose} className="inline-flex items-center justify-center rounded-lg text-xs font-bold border border-primary/10 bg-card/60 text-muted-foreground h-9 px-4 flex-1 cursor-pointer">Cancel</button>
