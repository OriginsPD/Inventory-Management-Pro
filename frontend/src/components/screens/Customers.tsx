          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button 
                ...
                className="inline-flex items-center justify-center rounded-lg text-xs font-bold transition-all border border-primary/10 bg-primary/5 hover:bg-primary/15 text-[#d8e2fd] h-9 w-9 cursor-pointer"
              >
                <span className="material-symbols-outlined text-sm">keyboard_arrow_left</span>
              </button>
              <div>
                <h2 className="text-2xl font-extrabold tracking-tight text-[#d8e2fd]">{selectedCustomer.name}</h2>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {selectedCustomer.taxId && (
                    <span className="text-[10px] text-[#bec8ce] font-mono">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1 space-y-6">
              <div className="glass-panel p-6 rounded-2xl space-y-4">
                <h3 className="text-sm font-extrabold text-[#d8e2fd] flex items-center gap-2">
                  <span className="material-symbols-outlined text-sm text-primary">person</span>
                  Contact Information
                </h3>
                <div className="space-y-4">
                  <div className="flex items-start gap-3 text-xs">
                    <span className="material-symbols-outlined text-sm text-[#bec8ce] mt-0.5">mail</span>
                    <div className="space-y-0.5">
                      <p className="text-[9px] font-bold text-primary uppercase tracking-wider">Email</p>
                      <p className="text-[#d8e2fd] font-semibold">{selectedCustomer.email || 'No email provided'}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 text-xs">
                    <span className="material-symbols-outlined text-sm text-[#bec8ce] mt-0.5">phone</span>
                    <div className="space-y-0.5">
                      <p className="text-[9px] font-bold text-primary uppercase tracking-wider">Phone</p>
                      <p className="text-[#d8e2fd] font-semibold">{selectedCustomer.phone || 'No phone provided'}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3 text-xs">
                    <span className="material-symbols-outlined text-sm text-[#bec8ce] mt-0.5">location_on</span>
                    <div className="space-y-0.5">
                      <p className="text-[9px] font-bold text-primary uppercase tracking-wider">Address</p>
                      <p className="text-[#d8e2fd] whitespace-pre-wrap font-semibold leading-relaxed">{selectedCustomer.address || 'No address provided'}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="glass-panel rounded-2xl overflow-hidden shadow-sm">
                <div className="bg-[#0f1524]/60 p-4 border-b border-primary/10 flex items-center justify-between">
                  <h3 className="text-sm font-extrabold text-[#d8e2fd] flex items-center gap-2">
                ...
                <div className="p-0">
                  <div className="flex border-b border-primary/10 px-4">
                    ...
                    <button className="px-4 py-2.5 text-xs font-bold uppercase tracking-wider border-b-2 border-transparent text-[#bec8ce] hover:text-[#d8e2fd] cursor-pointer">
                      ...
                      history.dispatched.map((dev) => (
                        <div key={dev.id} className="p-4 flex items-center justify-between hover:bg-primary/5 transition-colors">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-sm font-mono font-bold text-[#d8e2fd]">{dev.identifier}</p>
                              <p className="text-[10px] text-primary uppercase font-extrabold">{dev.modelName}</p>
                            </div>
                          </div>
                          <div className="text-right space-y-1">
                            <div className="flex items-center gap-1.5 text-xs text-[#bec8ce] justify-end">
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-[#d8e2fd]">Customer Management</h2>
                  ...
                  <TableRow key={customer.id} className="group hover:bg-primary/5 transition-colors">
                    <TableCell className="truncate">
                      ...
                      >
                        ...
                        <span className="font-bold text-[#d8e2fd] truncate">{customer.name}</span>
                      </button>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-xs text-[#bec8ce] truncate">
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5 truncate">
                          <span className="material-symbols-outlined text-xs text-[#bec8ce]">mail</span>
                          {customer.email || '-'}
                        </div>
                        <div className="flex items-center gap-1.5 truncate">
                          <span className="material-symbols-outlined text-xs text-[#bec8ce]">phone</span>
                          {customer.phone || '-'}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-xs text-[#bec8ce] truncate">
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5 font-mono truncate text-[#d8e2fd]">{customer.taxId || '-'}</div>
                        <div className="flex items-center gap-1.5 truncate">
                          <span className="material-symbols-outlined text-xs text-[#bec8ce]">location_on</span>
                          {customer.address || '-'}
                        </div>
                      </div>
                    </TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="glass-panel-elevated max-w-lg w-full overflow-hidden border-0 p-0 animate-in fade-in zoom-in-95 duration-150" showCloseButton={true}>
          <DialogHeader className="p-6 border-b border-primary/10 bg-[#0f1524]/60 text-left space-y-0.5">
            <DialogTitle className="text-lg font-extrabold text-[#d8e2fd] p-0">{selectedCustomer ? 'Edit Customer Profile' : 'Register New Customer'}</DialogTitle>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Customer Name / Entity</label>
                  <input 
                    {...register('name')} 
                    placeholder="e.g. Acme Corp or John Doe" 
                    className="flex h-9 w-full rounded-lg border border-primary/10 bg-primary/5 px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/20 text-[#d8e2fd]"
                  />
                  {errors.name && <p className="text-[10px] text-red-400 font-bold">{errors.name.message as string}</p>}
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Type</label>
                  <Controller
                    control={control}
                    name="type"
                    render={({ field }) => (
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Tax ID / National ID</label>
                  <input 
                    {...register('taxId')} 
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Email Address</label>
                  <input 
                    {...register('email')} 
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Phone Number</label>
                  <input 
                    {...register('phone')} 
                ...
                <div className="col-span-2 space-y-1.5">
                  <label className="text-xs font-semibold text-[#bec8ce] block">Physical Address</label>
                  <textarea 
                    {...register('address')}
                    className="flex min-h-[80px] w-full rounded-lg border border-primary/10 bg-primary/5 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/20 text-[#d8e2fd]"
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-primary/10">
                ...
                <button 
                  ...
                  className="inline-flex items-center justify-center whitespace-nowrap rounded-lg text-xs font-bold transition-all bg-primary text-[#081326] shadow hover:brightness-110 h-9 px-6 gap-1.5 cursor-pointer"
                >
                  {isSubmitting ? (
